...
class Paddle(Sprite):
    def __init__(self, velocity, image, bounds_y, *groups):
        ...
        # Like original pong, we break this up into 8 segments 
        # from the edge angle (acute_angle) to pi/2 at the center
        # Changing acute_angle lets us change the extreme edge angle of the paddle.
        acute_angle = .125
        # Build the angles from acute_angle to the first 0.5 center value 
        # then append the values going from the
        # second center 0.5 value by using the values we just calculated reversed.
        angles = [acute_angle + (0.5-acute_angle)/3.0 * n for n in range(4)]
        angles += map(lambda x: 1 + x * -1, reversed(angles))
        # Final table is the output vector (x,y) of each angle
        self.bounce_table = [(math.cos(n*math.pi-math.pi/2.0), 
                            math.sin(n*math.pi-math.pi/2.0)) for n in angles]
        
    def update(self):
        self.rect.y = max(self.bounds_y[0], 
                min(self.bounds_y[1]-self.rect.height, 
...
class Score(Sprite):
    ...
    def set_score(self, value):
        ...
        values =list(map(int, reversed(str(self.score_value))))
