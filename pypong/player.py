...
class BasicAIPlayer(object):
    ...
    def update(self, paddle, game):
        # Dead simple AI, waits until the ball is on its side 
        #of the screen then moves the paddle to intercept.
        # A bias is used to decide which edge of the paddle 
        # is going to be favored.
        if (paddle.rect.x < game.bounds.centerx and \
        game.ball.rect.x < game.bounds.centerx) or \
        (paddle.rect.x > game.bounds.centerx and \
        game.ball.rect.x > game.bounds.centerx):
            delta = (paddle.rect.centery + \
                    self.bias * paddle.rect.height) - \
                    game.ball.rect.centery 
    ...
    def hit(self):
        self.hit_count += 1
        if self.hit_count > 6:
            # Recalculate our bias, this game is going on forever
            self.bias = random.random() - 0.5            
            self.hit_count = 0
